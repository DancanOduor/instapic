...
class Database:
    ...
    def table_names(self, fts4=False, fts5=False):
        ...
        sql = "select name from sqlite_main where {}".format(" AND ".join(where))
    ...
    def view_names(self):
        return [
            ...
            for r in self.conn.execute(
                "select name from sqlite_main where type = 'view'"
            ).fetchall()
        ]
    ...
    def triggers(self):
        return [
            Trigger(*r)
            for r in self.conn.execute(
                "select name, tbl_name, sql from sqlite_main where type = 'trigger'"
            ).fetchall()
        ]
...
{columns_sql}{extra_pk}
    ...
    def add_foreign_keys(self, foreign_keys):
        ...
        # Construct SQL for use with "UPDATE sqlite_main SET sql = ? WHERE name = ?"
        table_sql = {}
        ...
        with self.conn:
            ...
            for table_name, new_sql in table_sql.items():
                cursor.execute(
                    "UPDATE sqlite_main SET sql = ? WHERE name = ?",
                    (new_sql, table_name),
                )
...
class Queryable:
    ...
    def schema(self):
        return self.db.conn.execute(
            "select sql from sqlite_main where name = ?", (self.name,)
        ).fetchone()[0]
